...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE donations ADD COLUMN status TEXT DEFAULT 'Pending'")
        conn.commit()
...
def donate():
    if request.method == 'POST':
        ...
        cur.execute("""
        INSERT INTO donations(food, quantity, people, location, status)
        VALUES(?,?,?,?,?)
        """, (food, quantity, people, location, 'Pending'))
...
def accept_delivery(id):
    ...
    # SAVE TO SAVINGS
    cur.execute("""
    INSERT INTO savings(amount, details)
    VALUES(?, ?)
    """, (50, "Food Delivery Completed"))
...
def savings():
    ...
    # DELIVERY HISTORY - food, volunteer, location, status
